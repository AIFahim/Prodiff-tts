...
from utils.os_utils import link_file, move_file, remove_file
from utils.text_encoder import is_sil_phoneme, build_token_encoder
...
class BasePreprocessor:
    ...
    #     """
    #     item_names = [
    #         f'{spk_name}-{idx:03d}' for spk_name in ['SPK1', 'SPK2'] for idx in range(100)
    #     ]
    #     wav_fns = [f'{item_name}.wav' for item_name in item_names]
    #     txts = [f'{item_name}.txt' for item_name in item_names]
    #     return [
    #         {
    #             'item_name': item_name,
    #             'wav_fn': wav_fn,
    ...
    #             'spk_name': spk_name,
    #             'txt_loader': None,
    #         } for item_name, wav_fn, txt, spk_name in zip(item_names, wav_fns, txts, spk_names)
    #     ]
    ...
    def process(self):
        ...
        meta_data = self.meta_data()  # Call meta_data method and store the result
        print(" self.meta_data() ", meta_data)
        meta_data = list(tqdm(meta_data, desc='Load meta data'))
        ...
        # item_names = [d['item_name'] for d in meta_data]
        # item_names = [item_raw[0]['item_name'] for item_raw in meta_data]
        item_names = [item_raw['item_name'] for item_raw in meta_data]

        # print(" item_names[0] ",item_names[0])
        assert len(item_names) == len(set(item_names)), 'Key `item_name` should be Unique.'
        ...
        # for idx,item_, (item_id, item) in enumerate(zip(meta_data, multiprocess_run_tqdm(process_item, args, desc='Preprocess'))):
        #     if idx == 1:
        #         break
        #     if item is not None:
        #         # item_.update(item)
        ...
        for idx, (item_, item_id, item) in enumerate(zip(meta_data, range(len(meta_data)), multiprocess_run_tqdm(process_item, args, desc='Preprocess'))):
        # for idx, (item_, item_id, item) in enumerate(zip(meta_data, range(len(meta_data)), process_item(args))):
            if idx == 1:
                break
            if item is not None:
                item_.update(item)
        ...
        print(" ph_encoder, _word_encoder ",ph_encoder, _word_encoder)
        assert False
    ...
    def preprocess_first_pass(cls, item_name, txt_raw, txt_processor,
        ...
        if txt_loader is not None:
            txt_raw = txt_loader(txt_raw)
        # a , b = cls.txt_to_ph(txt_processor, txt_raw, preprocess_args)
        # print("..................................",b)
        # assert False
